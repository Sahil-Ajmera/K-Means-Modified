 ...
 * @author Mahe
 */
public class DAASampleTest1 {
    
    static float a = 0;

    private static class KMeansObject {
        Integer integer;
        boolean used = false;
        
        public KMeansObject(Integer data) {
            integer = data;
        }
    }
    /**
    ...
    public static ArrayList<KMeansObject> main2(ArrayList<KMeansObject> sampleData, int size) {
        ...
        for(int i = 0; i < size; i++) {
            continueWhile = true;
            while (continueWhile) {
                KMeansObject object = sampleData.get((int)(Math.random() * sampleData.size()));
                if(!object.used) {
                    object.used = true;
        ...
        for(int i = 0; i < size; i++) {
            list.add(new ArrayList<KMeansObject>());
        }
        ...
        while(continueWhile) {
            ...
            for(int i = 0; i < list.size(); i++) {
                list.get(i).clear();
                list.get(i).add(centroids.get(i));
            }
            
            ArrayList<KMeansObject> newCentroids = KMeansAlgo(sampleData, list);
            
            continueWhile = false;
            
            for(int i = 0; i < centroids.size(); i++) {
                if(!centroids.get(i).integer.equals(newCentroids.get(i).integer)) {
                    continueWhile = true;
                    break;
                }
            }
            
            for (KMeansObject sampleData1 : sampleData) {
                sampleData1.used = false;
            }
            
            centroids = newCentroids;
        }
        float sumSilhouette = 0.0f, totalElements = 0.0f;
        for (int i = 0; i < list.size(); i++) {
            for (int j = 0; j < list.get(i).size(); j++) {
                sumSilhouette += silhouette(list, i, j);
            }
        }
        for(ArrayList<KMeansObject> childList : list) {
            totalElements += childList.size();
        }
        float answer = sumSilhouette / totalElements;
        a = answer;
        return centroids;
    }
    
    public static ArrayList<KMeansObject> KMeansAlgo(ArrayList<KMeansObject> sampleData, ArrayList<ArrayList<KMeansObject>> list) {
        for(int i = 0; i < sampleData.size(); i++) {
            KMeansObject object = sampleData.get(i);
            if(!object.used) {
                object.used = true;
                int minDistance = Math.abs(list.get(0).get(0).integer - object.integer);
                int index = 0;
                for(int j = 1; j < list.size(); j++) {
                    int distance = Math.abs(list.get(j).get(0).integer - object.integer);
        ...
        ArrayList<KMeansObject> newCentroids = new ArrayList<>();
        for(int i = 0; i < list.size(); i++) {
            int sum = 0;
            for(int j = 0; j < list.get(i).size(); j++) {
                sum += list.get(i).get(j).integer;
            }
            newCentroids.add(new KMeansObject(sum / list.get(i).size()));
        }
            
        return newCentroids;
    }
    //ask if objects can be in more than one cluster.Any other method for grouping apart from calculation of cluster head using median appraoch
    static float silhouette(ArrayList<ArrayList<KMeansObject>> list, int i, int j) {
        float ao = 0;
        ArrayList<Float>bo = new ArrayList<>();
        for (int k = 0; k < list.get(i).size(); k++) {
            if (k != j) {
                ao += Math.abs(list.get(i).get(j).integer - list.get(i).get(k).integer);
            }
        }
        if (list.get(i).size() == 0)
            return 0;
        ao = ao / (list.get(i).size() - 1);
        
        for (int k = 0; k < list.size(); k++) {
            float tempBo = 0;
            if (k != i) {
                for (int l = 0; l < list.get(k).size(); l++) {
                    tempBo += Math.abs(list.get(i).get(j).integer - list.get(k).get(l).integer);
                }
                bo.add(tempBo / list.get(k).size());
            }
        }
        float finalBo = Collections.min(bo);
        float b = (finalBo - ao) / Math.max(ao, finalBo);
        return b;
    }
}